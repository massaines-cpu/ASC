"""Lance l'expérience LODO SignalJEPA PreLocal proche du tutoriel officiel.
...
DATASET_VERSION = "data_signal_jepa_prelocal_19ch_128hz_2s_uv"
...
# L'early stopping sécurise le run, même si dix epochs restent rapides.
...
# Les futurs runs sont enregistrés dans la même expérience MLflow que les
# autres modèles ASC. Mettre False pour un lancement volontairement hors ligne.
ENABLE_MLFLOW = True
...
def train_one_fold(
    ...
    best_model_path = (
        models_dir / f"best_model_fold_{validation_dyad}.pt"
    )
    ...
    for epoch_index in range(config.number_of_epochs):
        ...
        tracker.log_epoch_metrics(
            ...
            metrics={
                ...
                "validation_window_loss": validation_metrics[
                    "window_loss"
                ],
        ...
        # Le checkpoint est choisi sur la loss participant : c'est l'unité
        # d'évaluation scientifique définie dans le projet ASC.
    ...
    summary = {
        ...
        "final_train_window_accuracy": history[
            "train_window_accuracy"
        ][-1],
        "best_checkpoint_participant_loss": final_metrics[
            "participant_loss"
        ],
        ...
        "best_checkpoint_window_accuracy": final_metrics[
            "window_accuracy"
        ],
    }
...
def main() -> None:
    ...
    if not config.dataset_root.exists():
        raise FileNotFoundError(
            ...
            "Exécuter d'abord "
            "python -m src.dataset.prepare_signal_jepa_prelocal"
        )
    ...
    for validation_dyad in config.selected_folds:
        ...
        with tracker.fold_run(
            run_name=(
                f"{config.experiment_name}__fold_{validation_dyad}"
            ),
    ...
    mean_accuracy = summary_table[
        "best_checkpoint_participant_accuracy"
    ].mean()
    mean_loss = summary_table[
        "best_checkpoint_participant_loss"
    ].mean()
    mean_window_accuracy = summary_table[
        "best_checkpoint_window_accuracy"
    ].mean()
    mean_window_loss = summary_table[
        "best_checkpoint_window_loss"
    ].mean()
    ...
    if len(summary_table) > 1:
        ...
        std_window_accuracy = summary_table[
            "best_checkpoint_window_accuracy"
        ].std()
        std_window_loss = summary_table[
            "best_checkpoint_window_loss"
        ].std()
    else:
        std_accuracy = 0.0
    ...
    print("\n" + "=" * 72)
    print("RÉSUMÉ GLOBAL — SIGNALJEPA PRELOCAL")
    print("=" * 72)
