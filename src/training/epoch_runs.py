        ...
        for eeg, labels in loader:
            ...
            if training:
                ...
                if total_examples == 0:
                    ...
                    # --------------------------------------------------
                    # 1. Vérification des gradients
                    # --------------------------------------------------

                    gradient_linear = (
                        model.final_layer.linear.weight.grad
                    )

                    print(
                        "Gradient Linear mean :",
                        gradient_linear.abs().mean().item(),
                    )

                    print(
                        "Gradient Linear max  :",
                        gradient_linear.abs().max().item(),
                    )

                    gradient_conv = (
                        model.final_layer.spat_conv.weight.grad
                    )

                    print(
                        "Gradient Conv3d mean :",
                        gradient_conv.abs().mean().item(),
                    )

                    print(
                        "Gradient Conv3d max  :",
                        gradient_conv.abs().max().item(),
                    )

                    # --------------------------------------------------
                    # 2. Sauvegarde des poids avant optimizer.step()
                    # --------------------------------------------------
                ...
                if total_examples == 0:
                    ...
                    print(
                        "Modification poids mean :",
                        weight_change.mean().item(),
                    )

                    print(
                        "Modification poids max  :",
