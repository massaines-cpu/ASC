"""Prépare le dataset ASC pour SignalJEPA PreLocal, montage complet 32 canaux.

Variante de ``prepare_signal_jepa_prelocal.py`` sans réduction de montage :
    ...
    5 fenêtres non chevauchantes de 2 secondes, soit 256 points
                              ↓
    [2 participants, 5 fenêtres, 32 canaux, 256 points]

Pourquoi cette variante existe
-------------------------------
La version à 19 canaux réduisait le montage pour suivre le tutoriel
Braindecode d'origine, pas parce que PreLocal l'exige : PreLocal ne réutilise
que le ``feature_encoder`` convolutif du checkpoint SSL, sans table
d'embeddings de canaux ni transformer (contrairement à
``SignalJEPA_Contextual``, dont le déséquilibre structurel position/contenu
sur ces données est documenté dans la docstring de
``src/models/signal_jepa_model.py``). Rien dans PreLocal ne dépend donc d'un
nombre de canaux précis : ce script permet d'utiliser les 32 canaux ASC
complets, comme demandé, avec l'architecture qui a déjà démontré qu'elle
apprend sur ces données (accuracy dès la première epoch en scratch).
...
TARGET_DATASET_NAME = "data_signal_jepa_prelocal_32ch_128hz_2s_uv"

# Même conclusion que prepare_signal_jepa.py — voir docstring ci-dessus.
APPLY_BANDPASS = True
...
TARGET_CHANNELS = 32  # Aucune réduction : les 32 canaux ASC sont conservés.
TARGET_WINDOW_TIMEPOINTS = 256
TARGET_NUMBER_OF_WINDOWS = 5

MICROVOLTS_PER_VOLT = 1_000_000.0

# Ordre réel des canaux ASC. Identique à prepare_signal_jepa_prelocal.py,
# prepare_signal_jepa.py et src/eeg_model_registry/config.py.
ASC_CHANNEL_NAMES = [
...
def validate_configuration() -> None:
    ...
    if TARGET_CHANNELS != EXPECTED_SOURCE_CHANNELS:
        raise ValueError(
            "Cette variante ne réduit pas le montage : TARGET_CHANNELS doit "
            "valoir 32."
        )
...
def transform_one_file(eeg: np.ndarray) -> np.ndarray:
    ...
    # Pas de sélection de canaux ici : les 32 sont conservés tels quels.
